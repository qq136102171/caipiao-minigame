...
> **最近会话**: 2026-08-17（大乐透金额修复 + 倍率设置 v1.5.0）
> **当前版本**: v1.5.0（已上传微信后台，待提交审核）
...
```
版本: 1.5.0 已上传 (274.9 KB, ✔ upload @ 44132, 大乐透金额修复 + 倍率设置)
...
## 29. v1.5.0 本次会话（2026-08-17）

### 问题（用户反馈）

1. 大乐透生成后显示 12 元，实际购买只要 8 元
2. 需要新增「倍率（倍投）」设置，按实际购买倍率调整金额

### 根因

`utils/dlt.js` 写死 `totalCost: 12 // 4 注 × 3 元`。大乐透基础价是 **2 元/注**（4 注 = 8 元），3 元是「追加」价，不能当基础价。

顺带发现一个连带 bug：SSQ 蓝球命中判断存在数据格式不一致 —— 本地历史蓝球是数组 `[5]`，网络接口是数字 `5`，导致蓝球命中经常漏算（如 5+1 会被算成 5+0）。此前票大多「未中奖」没暴露。

### 修复

1. **`utils/dlt.js`**：`totalCost: 12 → 8`
2. **倍率设置（game.js）**：
   - 票面新增「倍率 1/2/3/5/10 倍」选择行（导出/保存按钮下方），点击即时重算金额（号码不变）
   - state 新增 `multiplier`（默认 1）+ `baseCost`（单倍金额）；`totalCost = baseCost × multiplier`
   - 底部汇总显示 `16 元 ×2`；导出清单加「倍率：N 倍」行
   - 保存票时记录 `multiplier`；列表/详情显示 `×N倍`
3. **奖金按倍率计算（library.js）**：`checkItem` 中 `prizeAmount × (item.multiplier || 1)`，倍投票盈亏记录才准；旧票无 multiplier 默认 1 倍
4. **蓝球格式兼容（library.js `_hitCount`）**：SSQ/DLT 的 secondary 兼容 `[5]` 与 `5` 两种格式
...
|---|---|---|
| JS 语法 | `node -c` | ✅ 全过 |
| 金额 | Node 单测 | ✅ DLT 4注=8元 / SSQ 6注=12元 |
| 倍率奖金 | Node 单测 | ✅ 倍率 2 票六等奖 10 元；旧票默认 5 元 |
| 蓝球命中 | Node 单测 | ✅ 历史 `[5]` 与网络 `5` 两种格式都命中 |
| 端到端 | canvas/wx mock 真实触摸流 | ✅ DLT→生成→选2倍→保存 = 4注/16元/倍率2 |
| 回归 | mock 打开彩票库+刷新 | ✅ 无崩溃 |
| 上传 | CLI @ 44132 | ✅ v1.5.0，274.9 KB |

### 关键设计原则（新增）

> 金额 = 单倍基础价 × 倍率。`generator/dlt` 只返回**单倍** `totalCost`，倍率由 game.js 统一乘，避免生成器里出现「×3 元」这类硬编码错误。
